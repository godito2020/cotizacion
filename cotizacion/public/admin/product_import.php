<?php
// cotizacion/public/admin/product_import.php
...
if (!$auth->hasRole('Company Admin')) {
    $_SESSION['error_message'] = "No está autorizado para importar productos.";
    $auth->redirect(BASE_URL . '/admin/index.php');
}

$userRepo = new User(); // For header display of roles

$page_title = "Importar Productos desde CSV";
$import_results = null; // To store messages and errors

// Define expected CSV headers
$expectedHeaders = ['SKU', 'Nombre', 'Precio', 'Descripción']; // Matches typical output of database/export

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['product_file'])) {
    if ($_FILES['product_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedfileExtensions)) {
            // It's a CSV file, proceed with parsing
            ...
            if (isset($parsedData['error'])) {
                ...
                foreach ($rowsToImport as $row) {
                    $rowNumber++; // For user-friendly error messages (row 2 in file is $rowNumber 2)
                    $sku = trim($row['SKU'] ?? '');
                    $nombre = trim($row['Nombre'] ?? '');
                    $precio_str = trim($row['Precio'] ?? '');
                    $descripcion = trim($row['Descripción'] ?? null);
                    if (empty($descripcion)) $descripcion = null;


                    // Validate data for each row
                    $current_row_errors = [];
                    if (empty($sku)) {
                        $current_row_errors[] = "SKU es requerido.";
                    }
                    if (empty($nombre)) {
                        $current_row_errors[] = "Nombre es requerido.";
                    }
                    if (empty($precio_str)) {
                        $current_row_errors[] = "Precio es requerido.";
                    } elseif (!is_numeric($precio_str) || floatval($precio_str) < 0) {
                        $current_row_errors[] = "Precio debe ser un número positivo. Valor encontrado: '" . htmlspecialchars($precio_str) . "'.";
                    }

                    $precio = floatval($precio_str);

                    if (empty($current_row_errors) && !$productRepo->isSkuUniqueForCompany($sku, $company_id)) {
                        $current_row_errors[] = "SKU '" . htmlspecialchars($sku) . "' ya existe en su compañía.";
                    }
                    ...
                    if (!empty($current_row_errors)) {
                        ...
                        $newProductId = $productRepo->create($company_id, $nombre, $sku, $precio, $descripcion);
                        if ($newProductId) {
                            $importedCount++;
                        } else {
                            $skippedCount++;
                            $rowErrors[] = "Fila " . $rowNumber . ": Error al guardar producto '" . htmlspecialchars($nombre) . "' (SKU: " . htmlspecialchars($sku) . "). Verifique los logs del servidor.";
                        }
                    }
                }
                $import_results['success_summary'] = $importedCount . " productos importados exitosamente.";
                if ($skippedCount > 0) {
    ...
    // No need to manually delete $fileTmpPath, PHP handles it after script execution.
}
    ...
    <div class="admin-container">
        <div class="admin-content">
            ...
            <div class="instructions">
                ...
                <ul>
                    <li>Seleccione un archivo CSV para importar productos.</li>
                    <li>El archivo CSV debe tener las siguientes columnas en este orden exacto: <strong>SKU, Nombre, Precio, Descripción</strong>.</li>
                    <li>La primera fila del archivo debe ser la cabecera con estos nombres de columna.</li>
                    <li>La columna "Descripción" es opcional; si un producto no tiene descripción, deje esa celda vacía.</li>
                    <li>Los precios deben ser números (ej. 19.99 o 25.5).</li>
                    <!-- <li><a href="path/to/plantilla_productos.csv" download>Descargar plantilla CSV</a></li> -->
                </ul>
            </div>

            <form action="product_import.php" method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="product_file">Archivo CSV de Productos:</label>

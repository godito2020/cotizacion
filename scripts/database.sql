...
CREATE TABLE `products` (
  `id` INT PRIMARY KEY AUTO_INCREMENT,
  `company_id` INT NOT NULL,
  `code` VARCHAR(100) NOT NULL, -- CODIGO from Excel
  `description` TEXT NOT NULL, -- DESCRIPCION from Excel
  `brand` VARCHAR(100) NULL, -- MARCA from Excel
  `balance` DECIMAL(10, 2) DEFAULT 0.00, -- SALDO from Excel
  `premium_price` DECIMAL(10, 2) DEFAULT 0.00, -- PREMIUM from Excel
  `regular_price` DECIMAL(10, 2) DEFAULT 0.00, -- PRECIO from Excel
  `last_cost` DECIMAL(10, 2) DEFAULT 0.00, -- ULTCOSTO from Excel
  `last_cost_date` DATE NULL, -- FECULTCOS from Excel
  `image_url` VARCHAR(255) NULL, -- IMAGEN from Excel
...
-- Table: bank_accounts
-- Stores bank account information for companies
CREATE TABLE `bank_accounts` (
  `id` INT PRIMARY KEY AUTO_INCREMENT,
  `company_id` INT NOT NULL,
  `bank_name` VARCHAR(100) NOT NULL,
  `account_type` VARCHAR(50) NOT NULL, -- Corriente, Ahorro, etc.
  `account_number` VARCHAR(50) NOT NULL,
  `cci` VARCHAR(20) NULL, -- Código de Cuenta Interbancaria
  `currency` VARCHAR(3) DEFAULT 'PEN', -- PEN, USD, EUR, etc.
  `is_active` BOOLEAN DEFAULT TRUE,
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (`company_id`) REFERENCES `companies`(`id`) ON DELETE CASCADE
);
...
-- Stores email configuration per company
CREATE TABLE `email_settings` (
  ...
  `smtp_password` VARCHAR(255) NOT NULL, -- Should be encrypted
...
-- Table: api_settings
-- Stores API configurations (SUNAT, RENIEC, etc.)
CREATE TABLE `api_settings` (
  `id` INT PRIMARY KEY AUTO_INCREMENT,
  `company_id` INT NULL, -- NULL for system-wide APIs
  `api_name` VARCHAR(50) NOT NULL, -- 'SUNAT', 'RENIEC', etc.
  `api_url` VARCHAR(255) NOT NULL,
  `api_key` VARCHAR(255) NULL,
  `api_secret` VARCHAR(255) NULL,
  `additional_config` JSON NULL, -- For extra configuration parameters
  `is_active` BOOLEAN DEFAULT FALSE,
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (`company_id`) REFERENCES `companies`(`id`) ON DELETE CASCADE,
...
-- Extended stock table to handle multiple warehouse columns from Excel
CREATE TABLE `product_warehouse_stock` (
  `id` INT PRIMARY KEY AUTO_INCREMENT,
  `product_id` INT NOT NULL,
  `company_id` INT NOT NULL,
  `warehouse_name` VARCHAR(100) NOT NULL, -- Column name from Excel (YP, PRODUCTORES, etc.)

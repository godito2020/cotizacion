...
<body>
                                    ...
                                    <div class="col-md-3">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select" id="price_includes_igv" name="price_includes_igv">
                                                ...
                                                <option value="plus_igv" <?= ($formData['price_includes_igv'] ?? 'included') === 'plus_igv' ? 'selected' : '' ?>>MÁS IGV (+18%)</option>
                                            </select>
    ...
    <script>
        ...
        function selectProductFromSearch(product) {
            ...
            if (targetPrice) targetPrice.value = price.toFixed(2);
...
Tiempo de entrega: 7 días hábiles.`;
        ...
        async function sendByWhatsApp() {
            ...
            try {
                ...
                const waUrl = whatsappResult.message_text
                    ? `https://wa.me/${whatsappResult.phone}?text=${encodeURIComponent(waInjectEmoji(whatsappResult.message_text))}`
                    : whatsappResult.whatsapp_url;
                window.open(waUrl, '_blank');

                // Success message
                setTimeout(() => {
                    alert('✅ WhatsApp abierto exitosamente. Verifica la ventana nueva para enviar el mensaje.');
                    // Redirect to view page
                    window.location.href = `<?= BASE_URL ?>/quotations/view.php?id=${quotationId}`;
                }, 500);
